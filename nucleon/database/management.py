...
CREATE_TABLE_RE = re.compile(r'CREATE\s+TABLE\s+(\w+)', re.I)
INSERT_RE = re.compile(r'INSERT\b', re.I)
...
class SQLScript(object):
    ...
    def make_initdb(self):
        cs = []
        for c in self.commands:
            mo = CREATE_TABLE_RE.match(c)
            if mo:
                cs.append('DROP TABLE IF EXISTS %s' % mo.group(1))
            cs.append(c)
        return SQLScript(cs)
    ...
    def make_reset(self):
        ...
        for c in self.commands:
            mo = CREATE_TABLE_RE.match(c)
            if mo:
                cs.append('DELETE FROM %s' % mo.group(1))
